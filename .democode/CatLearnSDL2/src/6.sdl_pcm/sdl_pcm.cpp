...
#include <SDL.h>
#include <catlearnsdl2/filesystem.h>
...
int main(int argc, char* argv[])
{
    ...
    // 打开PCM文件
    audio_fd = fopen(FileSystem::getPath("resources/44100_16bit_2ch.pcm").c_str(), "rb");
    if(!audio_fd)
    {
    ...
    spec.channels = 2; // 2通道
    spec.silence = 0; 
    spec.samples = 1024; // 23.2ms -> 46.4ms 每次读取的采样数量，多久产生一次回调和 saples
