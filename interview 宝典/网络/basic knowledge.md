...
##### tcp，udb server和client建立流程

* TCP（传输控制协议）和UDP（用户数据报协议）是计算机网络中常用的两种传输协议。建立TCP和UDP服务端（server）和客户端（client）之间的连接流程如下：
	* TCP服务器端建立连接流程：
		1. 创建套接字（socket）：服务器端创建一个套接字，并绑定到一个特定的端口上。
		2. 监听端口：服务器端开始监听绑定的端口，等待客户端连接请求。
		3. 接受连接：当服务器监听到客户端的连接请求时，使用accept()函数接受连接，并返回一个新的套接字，用于与该客户端通信。
		4. 通信：通过新的套接字与客户端进行通信，可以使用read()和write()等函数进行数据的读取和发送。
		5. 关闭连接：通信结束后，服务器可以使用close()函数关闭与客户端的连接。
	* TCP客户端建立连接流程：
		1. 创建套接字（socket）：客户端创建一个套接字。
		2. 发起连接请求：客户端使用connect()函数，向服务器端发送连接请求。
		3. 等待连接确认：客户端等待服务器端的连接确认。
		4. 通信：连接确认后，客户端和服务器端之间可以进行通信，使用read()和write()等函数进行数据的读取和发送。
		5. 关闭连接：通信结束后，客户端可以使用close()函数关闭与服务器端的连接。
	* UDP服务器端建立连接流程：
		1. 创建套接字（socket）：服务器端创建一个套接字，并绑定到一个特定的端口上。
		2. 接收数据：服务器端使用recvfrom()函数持续监听该端口，等待客户端发送的数据。
		3. 数据处理：当服务器端接收到客户端发送的数据后，可以对数据进行处理和响应。
		4. 关闭连接：通信结束后，服务器端可以使用close()函数关闭与客户端的连接。
	* UDP客户端建立连接流程：
		1. 创建套接字（socket）：客户端创建一个套接字。
		2. 发送数据：客户端使用sendto()函数向指定的服务器端发送数据。
		3. 等待响应：客户端等待服务器端的响应数据。
		4. 处理响应：客户端接收到服务器端的响应数据后进行处理。
		5. 关闭连接：通信结束后，客户端可以使用close()函数关闭与服务器端的连接。

需要注意的是，TCP是面向连接的可靠传输协议，确保数据按顺序和可靠地传输，但建立连接和传输过程的开销较大。而UDP是面向无连接的不可靠传输协议，不保证数据的可靠性和顺序，但具有较小的开销和延迟。在选择协议时，根据具体的需求和场景选择合适的协议。