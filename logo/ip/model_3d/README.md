# CatGray IP · v10 向前弧线招手

基于已确认的 v7 模型添加骨骼和动画。原静态工程、导出文件、脚本与预览完整保存在 `revisions/v7`。头部、加厚耳朵、身体比例、独立手臂、圆润腿根与白色椭圆腹纹均保留。

## 本次修复

- 上一版 v9 工程、GLB、视频、脚本和检查结果保存在 `revisions/v9`；更早版本仍保留。
- 招手重新设计为空间弧线：手臂向前伸出并抬起，手掌立起后左右轻摆两次，再沿弧线放下。
- 右臂改用 41 个连续截面骨骼，沿圆滑中心线保持横截面；重叠权重平滑相邻截面，消除原先多支点变形造成的波折和局部收窄。
- 抬手过程采用方向旋转插值，避免上下姿势直接混合时出现手臂缩短、塌陷。
- 保留骨骼不置顶、细线显示的设置。

## 已完成

- 84 根原生骨骼，其中 79 根启用变形，包含头身、双臂、腿脚、耳朵和七段尾骨。
- 全部 28 个角色网格完成归一化权重，每个顶点最多受四根骨骼影响。
- 头部、眼睛和鼻嘴共同跟随头骨，保持五官贴合。
- 双臂保留独立网格。左臂保留 FK；右臂使用 `ArmArc.R.00–40` 截面骨骼，`UpperArm.R` 可整体调整右臂。
- 双腿采用脚部 IK 控制和膝盖方向控制，走路支撑阶段脚底接地。
- 左肘、膝、踝辅助骨骼分散弯曲；右臂原有 Forearm/Hand 辅助骨骼保留但不驱动皮肤，并默认隐藏。
...
| `Clavicle.L/R`、`UpperArm.L/R` | 肩部及整条手臂的调整 |
| `Forearm.L`、`Hand.L` | 左侧前臂和手掌 FK |
| `ArmArc.R.00–40` | 右臂弧线的连续截面；在隐藏的辅助骨骼集合中启用后可编辑 |
| `CTRL_Foot_IK.L/R` | 脚的位置与朝向 |
| `CTRL_Knee_Pole.L/R` | 膝盖的弯曲方向 |
| `Ear.L/R` | 耳朵轻微摆动 |
| `Tail.01`–`Tail.07` | 尾巴分段控制 |

右臂的弧线形状由 `wave_arc.py` 生成并记录成普通骨骼关键帧；改变招手路径时可调整该脚本的控制点再重新生成，或直接编辑截面骨骼关键帧。辅助关节、右臂截面和腿部变形骨骼默认放在隐藏的骨骼集合中。原静态手臂对象的肩部原点仍保留，但绑定后应通过骨骼摆姿势。
...
| `animations/Wave.mp4` / `Wave_v10.mp4` | 斜侧面挥手；后者为带版本号的同内容预览 |
| `animations/Wave_Side.mp4` / `Wave_Side_v10.mp4` | 侧面挥手，检查向前弧度 |
...
| `qa/wave_revision_verification.json` | 招手全 97 帧表面自相交、截面面积、弧线连续性与骨骼显示检查 |
...
- `wave_arc.py`：右臂弧线、截面骨骼与连续运动插值。
- `render_wave_side.py`：生成并验证侧面招手视频。
