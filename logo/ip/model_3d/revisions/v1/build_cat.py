"""Reference-matched CatGray mascot. Blender 4.1+, no external packages.

Run: blender --background --factory-startup --python build_cat.py
Coordinates: X left/right, -Y front, Z up. One unit = 100 reference pixels.
...
eargray = material('Ear rims | reference #747474', '747474')
pink = material('Inner ears | reference #FFA3A3', 'FFA3A3', .6)
...
tex.filepath_raw=str(OUT/'CatGray_Fur_BaseColor.png')
tex.file_format='PNG'
tex.save()
...
tex=bpy.data.images.load(str(OUT/'CatGray_Fur_BaseColor.png'),check_existing=False)
tex.pack()
...
for sign,label in ((-1,'L'),(1,'R')):
    outline=[(sign*x,z) for x,z in [(1.05,4.03),(1.62,4.30),(1.72,4.28),(1.80,4.00),(1.76,3.43),(1.36,3.65)]]
    ear_y=lambda x,z: .12+.90*(abs(x)-1.4)
    shell=patch('Ear '+label+' • rounded outer shell',outline,ear_y,eargray,.065,14,dark,.015)
    solid=shell.modifiers.new('Ear volume','SOLIDIFY');solid.thickness=.18
    solid.offset=-1
    inner=[(sign*x,z) for x,z in [(1.26,4.07),(1.60,4.16),(1.65,4.12),(1.68,3.86),(1.65,3.65),(1.40,3.82)]]
    patch('Ear '+label+' • inset pink',inner,lambda x,z:ear_y(x,z)-.061,pink,.024,12,dark,.011)
...
# Small gray body and unshod paws. The gray neck is mostly inside the head.
ellipsoid('Body • gray neck',(0,.05,1.36),(.38,.32,.21),fur)
TORSO=[(.445,.53,-.35,.39),(.48,.63,-.42,.45),(.59,.66,-.46,.48),
       (.82,.665,-.47,.50),(1.03,.62,-.45,.48),(1.20,.58,-.42,.43),
       (1.33,.55,-.40,.44),(1.44,.45,-.37,.38)]
torso=ring_mesh('Hoodie • soft fitted torso',TORSO,cloth,power=2.65,nz=60,nt=112)

def torso_front(x,z,offset=.012):
    return front_surface(x,z,offset,TORSO,2.65)

for sign,label in ((-1,'L'),(1,'R')):
    leg=ellipsoid('Leg '+label+' • gray',(sign*.25,.015,.285),(.255,.29,.295),fur)
    ellipsoid('Paw '+label+' • small flat foot',(sign*.245,-.18,.070),(.231,.292,.090),fur)
    # Sleeve centerline and radius profile create a hanging, slightly bent arm.
    profiles=[(.53,sign*.708,-.006,.160,.245),(.59,sign*.721,-.008,.181,.253),
              (.77,sign*.698,.020,.191,.247),(1.01,sign*.603,.047,.182,.226),
              (1.18,sign*.500,.062,.148,.170),(1.24,sign*.46,.065,.065,.090)]
    verts=[];faces=[];n=64
    for z,x,y,wx,wy in profiles:
        for i in range(n):
            a=TAU*i/n
            verts.append((x+wx*math.cos(a),y+wy*math.sin(a),z))
    ...
    mesh('Sleeve '+label+' • relaxed arm',verts,faces,cloth,sub=2)
    ellipsoid('Hand '+label+' • gray mitten',(sign*.720,-.005,.524),(.156,.215,.157),fur)
    for h in (.622,.554):
        pts=[(sign*.718+.178*math.cos(TAU*i/64),-.009+.253*math.sin(TAU*i/64),h) for i in range(64)]
        curve('Cuff '+label+' • seam '+str(h),pts,.013,dark,True)

# Hood collar, the small white insert, and the folded hood on the back.
# A continuous cloth cowl connects the front folds to the back hood. The folds
# are surface details on this volume, so they remain attached in a side view.
vertices=[];faces=[];nn=128;rr=14
for j in range(rr+1):
    t=j/rr
    for i in range(nn):
        a=TAU*i/nn;s=math.sin(a);c=math.cos(a)
        xi=.29*s;yi=-.25*c
        xo=.714*s;yo=-.48*c if c>=0 else -.548*c
        zo=1.143+.235*abs(s)**.65 if c>=0 else 1.365
        vertices.append((xi+(xo-xi)*t,yi+(yo-yi)*t,1.43+(zo-1.43)*t+.025*math.sin(math.pi*t)))
for j in range(rr):
    for i in range(nn):
        k=j*nn+i;kn=j*nn+(i+1)%nn
        faces.append((k,kn,kn+nn,k+nn))
cowl=mesh('Hood • continuous shoulder cowl',vertices,faces,cloth)
solid=cowl.modifiers.new('Cloth thickness','SOLIDIFY');solid.thickness=.045
patch('Neckline • white triangular undershirt',[(-.24,1.327),(.24,1.327),(-.020,1.150)],
      lambda x,z:-.525,white,.006,8)
for sign,label in ((-1,'L'),(1,'R')):
    outline=[(sign*.025,1.125),(sign*.32,1.139),(sign*.696,1.215),
             (sign*.709,1.317),(sign*.563,1.403),(sign*.368,1.366),(sign*.156,1.247)]
    fold=patch('Hood • front fold '+label,outline,lambda x,z:-.492+.035*abs(x),cloth,.030,12,dark,.013)
    solid=fold.modifiers.new('Cloth volume','SOLIDIFY');solid.thickness=.065;solid.offset=-1

hood_outline=[(-.642,1.367),(-.415,1.419),(0,1.350),(.415,1.419),(.642,1.367),
              (.515,1.192),(.264,1.009),(0,.912),(-.264,1.009),(-.515,1.192)]
# Reverse the bulge direction for the back-facing hood.
hood=patch('Hood • back drape',hood_outline,lambda x,z:-.532,cloth,.083,14)
for v in hood.data.vertices: v.co.y=-v.co.y
for p in hood.data.polygons: p.flip()
solid=hood.modifiers.new('Soft hood volume','SOLIDIFY');solid.thickness=.11;solid.offset=-1
curve('Hood • back V hem',[(x,.553+.05*(1-abs(x)/.642),z) for x,z in hood_outline],.014,dark,True)

def front_line(name,xzs,r=.012,mat=dark):
    return curve(name,[(x,torso_front(x,z,.023),z) for x,z in xzs],r,mat)

front_line('Hoodie • zipper center',[(-.037,.478),(-.037,.78),(-.037,1.117)],.011)
front_line('Hoodie • placket edge',[(-.177,.49),(-.177,.80),(-.177,1.135)],.012)
for sign,label in ((-1,'L'),(1,'R')):
    pts=[(sign*.065,.727),(sign*.145,.650),(sign*.258,.604),(sign*.372,.604),(sign*.372,.493)]
    front_line('Pocket '+label+' • curved hand opening',pts,.010)
    front_line('Pocket '+label+' • bottom edge',[(sign*.372,.493),(sign*.205,.493),(sign*.065,.493)],.010)
for x,endz in ((-.17,.76),(.15,.82)):
    curve('Hoodie • drawcord '+str(x),[(x,-.528,1.14),(x,torso_front(x,1.0,.034),1.00),
                                    (x,torso_front(x,endz,.034),endz)],.014,dark)
for z in (.470,.526):
    pts=[ring_xyz(TAU*i/128,z,TORSO,2.65) for i in range(128)]
    pts=[(x*1.007,y*1.025,z) for x,y,z in pts]
    curve('Hoodie • lower rib seam '+str(z),pts,.012,dark,True)
...
ellipsoid('Tail • rounded tip',tail_points[-1],(.132,.132,.132),fur,48,32)

# Model root: select this object to move or resize the entire character.
for obj in list(model.objects):
    if obj.name.startswith(('Body','Hood','Neckline','Sleeve','Cuff','Hand','Paw','Leg','Pocket')):
        obj.scale.y*=1.28
        obj.location.y*=1.28
...
bpy.ops.mesh.primitive_plane_add(size=200,location=(0,0,-.032))
...
cams={
    'front':camera('Camera • Front',(0,-12,2.22),(0,0,2.22),5.14),
...
(OUT/'model_info.json').write_text(json.dumps(stats,ensure_ascii=False,indent=2),encoding='utf-8')
