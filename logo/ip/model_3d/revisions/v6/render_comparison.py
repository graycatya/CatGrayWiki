"""Render archived v5 and current v6 at an identical orthographic scale."""
import bpy
import math
...
labels = []
for x, label in ((-2.17, 'V5'), (2.17, 'V6')):
...
print('V5_V6_COMPARISON_COMPLETE', flush=True)

# A closer frontal comparison reveals the arm clearance and hip curvature.
current_root.location.x = 1.17
previous_root.location.x = -1.17
camera.location = (0, -18, .74)
camera.rotation_euler = (Vector((0, 0, .74))-camera.location).to_track_quat('-Z', 'Y').to_euler()
camera.data.ortho_scale = 4.78
for obj, x in zip(labels, (-1.17, 1.17)):
    obj.location = (x, -1.7, -.22)
    obj.data.size = .115
scene.render.resolution_y = 800
scene.render.filepath = str(OUT/'preview_body_comparison.png')
bpy.ops.render.render(write_still=True)
print('V5_V6_BODY_COMPARISON_COMPLETE', flush=True)
