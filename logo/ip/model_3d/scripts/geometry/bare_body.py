"""Bare CatGray torso/legs, separate rounded arms and painted oval belly.

Coordinates follow build_base.py: X left/right, -Y front, Z up. The neck is
...
# z, center X, center Y, X radius, Y radius. Follow the torso's convex flank
# down and inward; maintain a narrow air gap below the concealed shoulder.
# Each arm remains an independent closed quad loft for later deformation.
ARM_PROFILE = [
    (.485, .718, -.090, .002, .003),
    (.495, .718, -.089, .059, .077),
    (.530, .722, -.083, .110, .146),
    ...
    (.760, .839, -.028, .147, .210),
    (.890, .849, .000, .145, .219),
    (1.000, .837, .010, .145, .218),
    (1.130, .790, .022, .148, .215),
    (1.255, .687, .025, .166, .200),
    (1.370, .544, .029, .177, .184),
    (1.460, .457, .029, .123, .137),
    (1.505, .430, .029, .002, .003),
]
...
def build_body(*, mesh, curve, ellipsoid, patch, ring_mesh, interpolate,
    ...
    # Broader upper thighs overlap the lower abdomen. Relax this junction
    # locally so its highlight flows continuously down into each short leg.
    hip_group = body.vertex_groups.new(name='Hip transition')
    def fade(value):
        t = max(0.0, min(1.0, value))
        return t*t*(3-2*t)
    for vertex in body.data.vertices:
        x, y, z = vertex.co
        weight = fade((z-.16)/.15)*fade((.80-z)/.18)
        if weight > 0:
            hip_group.add([vertex.index], weight, 'REPLACE')
    relax = body.modifiers.new('Round abdomen into hips and legs', 'SMOOTH')
    relax.vertex_group = hip_group.name
    relax.factor = .75
    relax.iterations = 110
    apply_modifier(body, relax)
    cleanup_group = body.vertex_groups.get('Hip transition')
    ...
    # Never join or voxel-remesh the arms with the trunk. Shoulder origins
    # preserve the neutral pose and make each arm independently poseable.
    # The upper cap is seated inside the shoulder; exposed inner edges stay
    # clear of the torso. Bone/weight binding is intentionally a later step.
    from mathutils import Vector
    arms = []
    for sign, label in ((-1, 'L'), (1, 'R')):
        arm = loft('Arm ' + label + ' • independent rounded limb', ARM_PROFILE,
                   sign, sections=40, around=32)
        ...
        bm.to_mesh(arm.data)
        bm.free()
        pivot = Vector((sign*.535, .029, (1.375-bottom)*BODY_Z_SCALE))
        for vertex in arm.data.vertices:
            vertex.co -= pivot
        arm.location = pivot
        sub_arm = arm.modifiers.new('Rounded arm surface; retain editable quad loops', 'SUBSURF')
        sub_arm.levels = 2
        sub_arm.render_levels = 2
        arm['rigging_role'] = 'Independent arm skin; shoulder origin; no bones or weights yet'
        arm['shoulder_pivot'] = list(pivot)
        arm['elbow_landmark'] = [sign*.838, .0, (.94-bottom)*BODY_Z_SCALE]
        arm['wrist_landmark'] = [sign*.756, -.064, (.61-bottom)*BODY_Z_SCALE]
        arms.append(arm)
    ...
    body['surface_design'] = 'Connected torso and legs; separate arms; belly painted in basecolor'
    ...
    return {
        'revision': 'v6 close rounded independent arms and smooth hip/leg transition',
        'body_object': body.name,
        'arm_objects': [arm.name for arm in arms],
        'skin_connected_components': components,
        'skin_non_manifold_edges': non_manifold_edges,
        'evaluated_triangles': triangle_count,
        'voxel_size': VOXEL_SIZE,
        'width_with_arms': round(max(arm.location.x + max(v.co.x for v in arm.data.vertices)
                                     for arm in arms)*2, 4),
        'height': round(float(body.dimensions.z), 4),
        'torso_width_scale_from_v3': TORSO_X_SCALE,
        'body_height_scale_from_v3': BODY_Z_SCALE,
        'arm_attachment': 'Independent closed quad meshes, narrow flank clearance, concealed shoulder caps',
        'rigging_status': 'Shoulder origins and joint landmarks only; skeleton and weights not yet created',
        'belly': {
