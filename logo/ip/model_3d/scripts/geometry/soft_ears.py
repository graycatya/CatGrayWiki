...
def build_ears(*, mesh, closed_catmull, fur, pink, eargray):
    ...
    for sign, label in ((-1, 'L'), (1, 'R')):
        # Keep the front centre seated at its approved position, and add most
        # of the volume behind it. A single smooth ellipsoid depth profile
        # rounds the whole rim without a sharp front/back junction.
        vertices = [(sign*cx, plane(cx)+EAR_REAR_OFFSET-EAR_HALF_DEPTH+INNER_CUP_DEPTH, cz)]
        for phi in angles:
            radius = math.sin(phi)
            for p in perimeter:
                x = cx + (p.x-cx)*radius
                z = cz + (p.y-cz)*radius
                y = plane(x)+EAR_REAR_OFFSET-EAR_HALF_DEPTH*math.cos(phi)
                # A shallow inner cup joins the thick, softly rounded ear rim.
                if phi < pink_angle:
                    y += INNER_CUP_DEPTH*(1-(radius/pink_radius)**2)**2
                vertices.append((sign*x, y, z))
        rear = len(vertices)
        vertices.append((sign*cx, plane(cx)+EAR_REAR_OFFSET+EAR_HALF_DEPTH, cz))
        faces = []
        pink_faces = []
        ...
        ear['design'] = 'Full rounded ear volume; preserved frontal outline and pink proportions; hidden root'
        ear['shell_depth'] = 2*EAR_HALF_DEPTH
        ear['rear_offset'] = EAR_REAR_OFFSET
        ear['pink_radius'] = pink_radius
        ear['shell_color'] = '#747474'
        ear['inner_color'] = '#FFA3A3'
        result.append(ear)
    return {'revision': 'v7 thicker rounded ear backs with preserved front silhouette and colors',
            'objects': [obj.name for obj in result],
            'outline_curves': False, 'shell_thickness': 2*EAR_HALF_DEPTH,
            'rear_offset': EAR_REAR_OFFSET,
            'front_center_offset': EAR_REAR_OFFSET-EAR_HALF_DEPTH+INNER_CUP_DEPTH,
