...
with bpy.data.libraries.load(str(OUT/'source/CatGray_Base.blend'),link=False) as (src,dst):
    dst.objects=list(original_names)
previous=dict(zip(original_names,dst.objects))
archive=bpy.data.collections.new('QA approved base reference')
...
check('approved_base_rest_shape_preserved',max(r['max_vertex_error'] for r in rest_errors)<.00002,
      {'objects':rest_errors})
