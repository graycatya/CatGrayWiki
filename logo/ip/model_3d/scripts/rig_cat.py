...
FPS=24
bpy.ops.wm.open_mainfile(filepath=str(OUT/'source/CatGray_Base.blend'))
...
leg_collection=data.collections.new('辅助 • Arm sections / Leg IK')
for b in data.bones:
...
rig['release_status']='v10 final • approved 2026-09-15'
scene['release_version']='v10'
...
report={'revision':'v10','source':'source/CatGray_Base.blend',
...
manifest=json.loads((OUT/'source/base_info.json').read_text())
manifest['rig_revision']=report
manifest['release']={'version':'v10','status':'final','reference_images':'../references','rebuild_source':'source/CatGray_Base.blend'}
...
if '--no-export' not in sys.argv:
    import runpy
    runpy.run_path(str(OUT/'scripts/export_rig.py'),run_name='__main__')
