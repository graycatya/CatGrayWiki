...
`stb_image.h`是[Sean Barrett](https://github.com/nothings)的一个非常流行的单头文件图像加载库，它能够加载大部分流行的文件格式，并且能够很简单得整合到你的工程之中。`stb_image.h`可以在[这里](../../../resource/计算机图形学/https://github.com/nothings/stb/blob/master/stb_image.h)下载。下载这一个头文件，将它以`stb_image.h`的名字加入你的工程，并另创建一个新的C++文件，输入以下代码：
...
![textures2](../../../resource/计算机图形学/textures2.png)
...
![textures_funky](../../../resource/计算机图形学/textures_funky.png)
...
通过使用glUniform1i设置采样器，我们保证了每个uniform采样器对应这正确的纹理单元。你应该能得到下面的结果:

![textures_combined](../../../resource/计算机图形学/textures_combined.png)


你可能注意到纹理上下颠倒了! 这是因为OpenGL要求y轴0.0坐标是在图片的底部的， 但是图片的y轴0.0坐标通常在顶部。很幸运，stb_image.h 能够在图像加载时我们翻转y轴，只需要在加载任何图像前加入以下语句即可:
...
![textures_combined2](../../../resource/计算机图形学/textures_combined2.png)
